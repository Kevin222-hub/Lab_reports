...
def SigmaM(x1,x2,y1,y2,sx1 = 0,sx2 = 0,sy1 = 0,sy2 = 0):
...
def SigmaQ(x1,x2,y1,y2,sx1 = 0,sx2 = 0,sy1 = 0,sy2 = 0):
...
def Retta(x1,x2,y1,y2,sx1 = 0,sx2 = 0,sy1 = 0,sy2 = 0):
    m = (y2-y1)/(x2-x1)
    q = y1 - m*x1
    sm = SigmaM(x1,x2,y1,y2, sx1 = sx1, sx2 = sx2, sy1 = sy1, sy2 = sy2)
    sq = SigmaQ(x1,x2,y1,y2, sx1 = sx1, sx2 = sx2, sy1 = sy1, sy2 = sy2)
    return m,sm,q,sq

def single(Value,x1,x2,y1,y2,sx1 = 0,sx2 = 0,sy1 = 0,sy2 = 0):
    m,sm,q,sq = Retta(x1,x2,y1,y2, sx1 = sx1, sx2 = sx2, sy1 = sy1, sy2 = sy2)
    X = (Value-q)/m
    sX = sqrt_sum( 1/m*sq, ((Value-q)/m**2)*sm) # da rifare covarianza m e q
    return X,sX
    ...
    for i in range(0,N):
        ...
        if i != N-1:
            if y[i] < Value and y[i+1] > Value:
                ...
                T,S = single(Value,x[i],x[i+1],y[i],y[i+1],sx1 = sx1, sx2 = sx2, sy1 = sy1, sy2 = sy2)
                X = np.append(X,T)
                sX = np.append(sX,S)
            ...
            if y[i] > Value and y[i+1] < Value:
                ...
                T,S = single(Value,x[i+1],x[i],y[i+1],y[i],sx1 = sx1, sx2 = sx2, sy1 = sy1, sy2 = sy2)
                X  = np.append(X,T)
                sX = np.append(sX,S)
    if plot:
        plt.plot(x,y,'.')
